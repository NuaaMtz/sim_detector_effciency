#!/usr/bin/env python3
"""Overlay energy spectra at fixed R for multiple Z values."""
...
def collect_files_for_fixed_r(
    data_dir: Path, fixed_r: float, atol: float
    ...
    for root_file in sorted(data_dir.glob("myfilename_R*p*_Z*p*.root")):
        ...
        if parsed is None:
            continue
        r_val, z_val = parsed
        if np.isclose(r_val, fixed_r, atol=atol, rtol=0.0):
            selected.append((root_file, z_val))
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Plot overlaid energy spectra for fixed R and varying Z."
    )
    parser.add_argument(
        "--data-dir",
        default="build",
    ...
    parser.add_argument(
        ...
        type=float,
        default=1,
        help="Fixed R value used to filter files (same unit as filename).",
    )
    parser.add_argument(
        "--r-atol",
        type=float,
        default=1e-6,
        help="Absolute tolerance when matching R from filename.",
    )
    parser.add_argument(
        "--bins",
        type=int,
    ...
    parser.add_argument(
        "--output",
        default=None,
        help="Output figure path. Default: spectrum_fixR_<R>.png",
    )
    return parser
...
def main() -> int:
    ...
    matched = collect_files_for_fixed_r(data_dir, args.fixed_r, args.r_atol)
    if not matched:
        print(f"No files found for fixed R={args.fixed_r} in {data_dir}")
        return 1

    spectra: Dict[float, np.ndarray] = {}
    for root_path, z_val in matched:
        energy = read_energy_array(root_path)
        if energy.size == 0:
            print(f"Skip empty energy branch: {root_path.name}")
            continue
        spectra[z_val] = energy
    ...
    for z_val in sorted(spectra.keys()):
        plt.hist(
            spectra[z_val],
            bins=args.bins,
            range=(e_min, e_max),
    ...
    plt.title(f"Energy spectra at fixed R={args.fixed_r:g}")
    plt.xlabel("Energy")
    ...
    output = (
        ...
        if args.output
        else Path.cwd() / f"spectrum_fixR_{args.fixed_r:g}.png"
    )
